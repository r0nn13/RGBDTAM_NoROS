...
DenseMapping::DenseMapping()
{
    ...
    cv::Mat map_points(0,6,CV_32FC1);
    ...
    if (pdense_mapper->get_do_dense()> 0.5)
    {
        ...
        /*// PUBLISH SUPERPIXELS
         ...
         for (int i = 1; i < points_superpixels_all.rows; i++)
         {
             ...
             float b = points_superpixels_all.at<float>(i,1);
             float c = points_superpixels_all.at<float>(i,2);
             int aa = round(points_superpixels_all.at<float>(i,3));
             int bb = round(points_superpixels_all.at<float>(i,4));;
             int cc = round(points_superpixels_all.at<float>(i,5));
...
void calculate_superpixels_and_setdata(Images_class &images,  int reference_image)
{
    ...
    chdir("/home/viki/catkin_ws/src/rgbtam/ThirdParty/segment");
    ...
    images.supIm[i]   -> set_data (R1,t1,f1);
    chdir(curre_dir);
}
...
void calculate_3D_superpixels_from_semidense(float limit_ratio_sing_val,float limit_normalized_residual,Images_class &images, cv::Mat &points6, int reference_image, float mean_value)
{
    ...
    for (int i = 0; i<= images.supIm[reference_image]->getNumberOfSuperpixels()-1  ;i++)
    {
        ...
        bool point_already_used[points6.rows];
        ...
        float points_sup_rows =  points_sup_total.rows;
